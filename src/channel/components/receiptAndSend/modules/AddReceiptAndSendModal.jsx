...
class AddReceiptAndSendModal extends Component{
    _retInitialState = ()=>{
        return {
            confirmLoading: false,
            spinning: false,                            // 数据加载loading状态标识符
            tip: '',                                    // 数据加载loading状态文本
            fileList: [],
            listPage: {
                pageIndex: 1,
    ...
    * */
    getOrderDetails = ({orderId}, callback) =>{
        fetch('ciq.receiveandsend.ready', {
            // method: 'post',
            // headers: {},
            data: {orderId},
            success: (res)=>{
    ...
    selectOrderCallback = (info)=>{
        ...
        if(click === 'ok'){
            ...
            if(data['id'] !== undefined || data[id] !== null){
                this.getOrderDetails({orderId: data["id"]}, (err, res)=>{
                    if(err){
                    ...
                    const {responseObject = {}} = res;
                    const {opUsers, opTime, coldchainType, remark, logisticsId, waybillCode, storeLocation, details = []} = responseObject;
                    this.props.form.setFieldsValue({
                        logisticsId,
                        waybillCode,
                        coldchainType,
                        opUsers,
                        opTime: moment(opTime),
                        storeLocation,
                        remark,
                    })
                    this.setState(({listPage})=>{
                        return {
                            listPage: {
                                ...listPage,
                                rows: this.state.listPage.rows.concat(details || [])
                            }
                        }
                    })
                })
    ...
    render(){
        ...
        const {spinning, tip, listPage: {pageIndex, pageSize, rows, total}, logisticsPage: {rows: logistics}, tableLoading,
...
AddReceiptAndSendModal.propTypes = {
    data: PropTypes.any,
...
AddReceiptAndSendModal.defaultProps = {
    prefix: 'yzh',
    visible: false,
    callback: noop,
};

AddReceiptAndSendModal = Form.create()(AddReceiptAndSendModal);

export default AddReceiptAndSendModal;