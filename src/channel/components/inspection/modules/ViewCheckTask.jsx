...
import {Form, Select, Input, Upload, Alert, Popconfirm, message, Spin, Icon, Modal, Button, DatePicker} from 'antd'
...
const {custom: {level}, order: {orderType}, tms: {tempType, transportType}, inspection: {checkType}, product: {itemTypes, countUnit, weightUnit, valueUnit}, custom: {entryExit}} = dimsMap;

function noop(){}

class ViewCheckTask extends Component{
    ...
    * */
     ...
     * @param {func} 回调函数
     * */
    viewCheckTask = (checkTaskId = {}, callback = ()=>{})=>{
    ...
    saveCheckPlaneTime = (data, callback)=>{
        fetch('ciq.checktask.modifyplantime', {
            // method: 'post',
            // headers: {},
            data: data,
            success: (res)=>{
    ...
    /*
    * @interface 保存修改后的查验时间
    * */
    saveCheckPlaneTimeClick = ()=>{
        this.props.form.validateFieldsAndScroll((err, values) => {
            if (err) {
                return;
            }
            ...
            const {id: checkTaskId} = data;
            const {planTime} = values;
            this.saveCheckPlaneTime({
                planTime: planTime.unix() * 1000,
                id: checkTaskId
            }, (err, res)=>{
                // case1. 保存查验时间失败
                if(err){
                    message.error('修改计划查验时间失败')
                    return;
                }

                // case2. 保存查验时间成功，设置为不可编辑状态，刷新查验任务明细
                message.success('修改计划查验时间成功');
                this.setState({
                    editable: false
                });
                this.viewCheckTask({id: checkTaskId}, (err, res)=>{
                    if(err){
                        return
                    }
    ...
    * */
    ...
    render(){
        const {prefix, title, visible, style, ...props} = this.props;
        const { getFieldDecorator } = this.props.form;
        const {confirmLoading, editable, btnLoading, data,
            displayOrderStatusModal, listPage: {pageIndex, pageSize, rows: dataSource, total}} = this.state;
        ...
        const rows = [
            ...
            [
                ...
                },
                {
                    ...
                    formatter: (gridMetaData)=>{
                        return (
                            <span>{checkType[data.checkType]}</span>
                        )
                    }
                },
                {
                    title: '计划查验时间',
                    rowHeight: editable ? 90 : 36,
                    span: 3
                },
                {
                    title: '计划查验时间',
                                ...
                                {editable ?
                                    <span>
                                        ...
                                        <Button
                                            style={{marginLeft: 4}}
                                            loading={btnLoading}
                                            onClick={this.saveCheckPlaneTimeClick}
                                            type="primary"
                                            size="small">确定</Button>
                                        <Button
                                            style={{marginLeft: 4}}
                                            onClick={this.triggerEditable}
                                            size="small">取消</Button>
                                    </span> :
                                    <span>
                                        {data.planTime ? moment(data.planTime).format("YYYY-MM-DD HH:mm:SS") : ''}
                                        <Button
                                            style={{marginLeft: 8}}
                                            onClick={this.triggerEditable}
                                            type="danger"
...
ViewCheckTask.defaultProps = {
    prefix: 'yzh',
    visible: false,
    callback: noop,
};

export default Form.create()(ViewCheckTask);
