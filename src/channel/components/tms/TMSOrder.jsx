...
import {TMSOrderQueryForm, AddTMSOrderModal, ViewTMSOrderModal,
    TMSOrderDetailModal, DispatchTMSOrderModal} from './moduels/'
...
class TMSOrder extends Component{
    ...
    retCurrentQuery = (oldQuery, newQuery)=>{
        if(isQueryChanged(oldQuery, newQuery)){
            this.state.listQuery = newQuery;
            return newQuery
        }else{
            return oldQuery
        }
     ...
     * */
    getTMSOrderDetails = ({data}, callback)=>{
        fetch('tms.waybill.detail', {
            // method: 'post',
            // headers: {},
            data: {id: data.id},
            success: (res)=>{
     ...
     * @interface 提交
     * */
    submitWayBill = ({data, callback})=>{
        fetch('tms.waybill.save', {
            // method: 'post',
            // headers: {},
            data: data,
            success: (res)=>{
            ...
            beforeSend: ()=>{
                this.setState(({addTMSOrderModal})=>{
                    return {
                        addTMSOrderModal: {
                            ...addTMSOrderModal,
                            confirmLoading: true
    ...
    _hasSelectedOnlyOneIdx = (selectedIds = [], nullText = undefined, moreText = undefined)=>{
        if(selectedIds.length === 0){
            ...
            message.warn(`${nullText || '你未选中任何运输单'}`);
            return false
        }else if(selectedIds.length > 1 ){
            // s1-case2. 选中超过2行数据提示
            message.warn(`${moreText || '你只能选择一条运输单'}`);
            return false;
        }
        return true
    };
    ...
    add = ({type, data})=>{
        ...
        if(type === 'edit'){
            this.getTMSOrderDetails({data}, (err, res)=>{
                if(err){
                    message.error(err.message);
                    return;
                }
    ...
    addCallback = (info)=>{
        ...
        if(click === 'ok'){
            console.log(info)
            this.submitWayBill({data, callback})
            return;
        }
    };
    ...
    render() {
        ...
        const {selectedIds, selectedRows, listPage: {pageIndex, pageSize, rows: dataSource, total},
            tableLoading,addTMSOrderModal, viewTMSOrderModal,
        ...
        const _pagination = {
            ...
            onChange: (current) => {
                this.getTMSOrderList({pageIndex: current, pageSize})
            }
        };
        ...
        const _columns = [
            ...
            },
            {
                ...
                formatter: ({dependentValues,rowIdx, column})=> {
                    ...
                    const operaCls = cx({
                        [`hidden`]: this.hiddenEleByStatus(0, dependentValues['status'])
                    });
                    const style={marginRight: 8}
                    return (
                        <span>
                            <a className={operaCls}
                               style={style}>
                                <Button
                                    onClick={this.add.bind(this, {type: 'edit', data: dependentValues})}
                                    title={`修改`}
                                    type="primary"
                                    shape="circle"
                                    icon="edit" />
                            ...
                            <Popconfirm
                                placement="topLeft"
                                title={`你确定要删除这条记录吗？`}
                                onConfirm={this.delTMSOrderClick.bind(this, {data: dependentValues})}
                                okText="确定"
                                cancelText="取消">
                                <a className={operaCls}
                                   style={style}>
                             ...
                             <a className={operaCls}
                                style={style}>
                                 <Button
                                     onClick={this.displayDispatchTMSOrderModal.bind(this, dependentValues)}
                                     title={`派发运输单`}
                                     type="primary"
                                     shape="circle"
                                     icon="select" />
                            ...
                            <a>
                                <Button
                                    onClick={this.showReportModal.bind(this, dependentValues)}
                                    title={`冷链报告`}
                                    type="primary"
                                    shape="circle"
                                    icon="line-chart" />
            ...
            },
            {
                ...
                formatter: ({dependentValues,rowIdx, value, column: {key}})=>{
                    ...
                    return (
                        <span className={cls}>{dimsMap.tms[key][value]}</span>
                    )
                }
                /*events: {
                 onDoubleClick: function () {
        ...
        return (
            <div className = {`${cls} has-no-check-box-all`} {...props}>
                <div className="hidden">
                    ...
                    <ViewTMSOrderModal
                        {...viewTMSOrderModal}
                        callback={this.viewTMSOrderCallback}/>
                    <DispatchTMSOrderModal
                        {...dispatchTMSOrderModal}
                        callback={this.dispatchTMSOrderCallback}/>
                    <TMSOrderDetailModal
                        {...tMSOrderDetailModal}
                        callback={this.reportCallback}/>
                </div>
                <TMSOrderQueryForm callback={this.queryCallback}/>
                <ButtonContainer>
                    <AddButton onClick={this.add.bind(this, {type: 'add', data: {}})}>新建运输单</AddButton>
...
TMSOrder = Form.create()(authPropTypesWrapper(TMSOrder));
...
export default connect(mapStateToProps)(TMSOrder);