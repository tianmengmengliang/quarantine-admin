...
class CopyPreApprovalT3Modal extends Component{
    ...
    if(visible && newData !== oldData){
      ...
      if( userId ){
        ...
        for(let i=0; i<source.length;  i++){
          arrsource.push( `${source[i]} `);//补空格。
        }
        let us = users.split(" ");
        for(let i=0; i<us.length;  i++){
          arrusers.push( `${us[i]} `); //补空格。
        }
        let pt = port.split(" ");
        for(let i=0; i<pt.length;  i++){
          arrport.push( `${pt[i]} `); //补空格。
        }
  ...
  onOk = ( )=> {
    this.props.form.validateFieldsAndScroll((err, data) => { //数据。
      ...
      let filePaths = {};
      let files = ["path2", 'path3',"path4", 'path5', 'path6', 'path7', 'path8', 'path9', 'path10', 'path11', 'path12', 'path13', 'path14'];
      files.forEach((key)=>{
...
//-------------------- 动态增加-----------------------------------------
  ...
  render(){
    const { samples=[], shippingDate=[],source=[], users=[], port=[],
      item2,item3,item4,item5,item6,item7,item8,item9,item10,item11,item12,item13,item14,item15 }=this.state;
    const {prefix, title, visible, data = {}, ...props} = this.props;
    const { getFieldDecorator, getFieldValue, getFieldsValue } = this.props.form;
    const {confirmLoading} = this.state;
    //console.log('item15',item15);
    ...
    const _tr = samples.map((k,index) => { //k数组中的元素  index数组下标。
      return ( <tr>
        <td>
          <Form.Item  key={k}>
            {getFieldDecorator(`sampleName${index}`,{ initialValue: k['sampleName'],
        ...
        <table className={tableCls}>
          ...
          <tr>
            ...
            <td colSpan={6}>
              <table className={inlineTableCls}>
                <tr>
                  <td style={titleStyle1}><div className={`${inlineTrCls}`}>样品来源</div></td>
                  <td><div className={`${inlineTrCls}`}>
                    <Form.Item>
                      {getFieldDecorator('sourceCollection', {
                        ...
                        <CheckboxGroup options={["人源 ","动物源 ","合成 ","其它 "]}  />
                      )}</Form.Item>
                  </div></td>
                </tr>
                <tr>
                  <td style={titleStyle1}><div className={`${inlineTrCls}`}>样品用途 </div></td>
                  <td><div className={`${inlineTrCls}`}>
                    <Form.Item>
                      {getFieldDecorator('users', {
                        ...
                        <CheckboxGroup options={["生产加工 ","科研 ","临床 ","销售 ","其它 "]}  />
                      )}</Form.Item>
                  </div></td>
                </tr>
                <tr>
                  ...
                  <td><div className={`${inlineTrCls}`} >
                    <Form.Item>
                      {getFieldDecorator('port', {
                        ...
                        <CheckboxGroup options={["北京首都机场 ","上海虹桥或浦东机场 ",
                                                 "杭州萧山国际机场 ","宁波栎社国际机场 ","温州龙湾国际机场 "]}   />
                      )}
                    </Form.Item>
                  </div></td>
                </tr>
          ...
          <tr>
            ...
            <td colSpan={6}>
              <table className={inlineTableCls}>
                ...
                <tr>
                  <td style={titleStyle3}>
                    <div style={{float:"left",}}>（二）出入境特殊物品样品信息表
                ...
                <tr>
                  <td style={titleStyle3}>
                    <div style={{float:"left",}}>（十一）国家食品药品监督管理局新药临床实验批件(涉及新药临床研究是提供)
                    </div>
                    <div style={{float:"left",}}>
                      { item11===1 ?
                        <Form.Item>
          ...
          <tr>
            ...
            <td colSpan={6} style={titleStyle2}>
              <div style={{float:"left",}}>
                <a href="http://ciq.yizhijie.com/fileDownload/3.doc">（情况说明表下载）</a>
              </div>
              <div style={{float:"left",}}>
                { item15.length >0 ?
                  <Form.Item>
...
CopyPreApprovalT3Modal.propTypes = {
  data: PropTypes.any,
...
CopyPreApprovalT3Modal.defaultProps = {
  prefix: 'yzh',
  visible: false,
  callback: noop,
};

CopyPreApprovalT3Modal = Form.create()(CopyPreApprovalT3Modal);

export default CopyPreApprovalT3Modal;
