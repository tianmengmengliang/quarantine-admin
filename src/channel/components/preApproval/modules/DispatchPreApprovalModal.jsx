...
class DispatchPreApprovalModal extends Component{
    _retInitialState = ()=>{
      return {
  ...
  render(){
           ...
           {/* <Col span={6}>开始日期
               <Form.Item>
                 {getFieldDecorator('dateStart', {
                   initialValue: data.dateStart ? moment(data.dateStart/1000 ): undefined,
                   rules:  [
                     {required:true, message:'开始日期'},
                   ]
                 })(
                   <DatePicker
                     format="YYYY-MM-DD"
           ...
           <Col span={6}>结束日期
               <Form.Item>
                 {getFieldDecorator('dateEnd', {
                   initialValue: data.dateEnd ? moment(data.dateEnd): undefined,
                   rules:  [
    ...
    return (
      <div>
          ...
          >
            { _data }
            <p style={{color:'#f00'}}>(*首选专家为专家组组长)</p>
            <GridTable
...
DispatchPreApprovalModal.propTypes = {
  data: PropTypes.any,
...
DispatchPreApprovalModal.defaultProps = {
  prefix: 'yzh',
  rowKey: 'id',
  visible: false,
  callback: noop,
...
DispatchPreApprovalModal = Form.create()(DispatchPreApprovalModal);

export default DispatchPreApprovalModal;
