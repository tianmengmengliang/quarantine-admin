 ...
 *   此组件已被废弃。
 * */
class CountApproval2 extends Component{
...
//------------------------------------------- start 列表展示 ----------------------------------------
  ...
  handleGridSort= (sortColumn, sortDirection)=> {
      ...
      //解决延迟: setState方法中调用函数，这样函数就可以获取最新的state。
...
//------------------------------------------- start查看-----------------------------------------------
  ...
  selectByCondition = ({data: selectedRows})=>{
    ...
    if(this._hasSelectedOnlyOneIdx(selectedRows)){
      this.showAdd({data: {}});
      this.viewCheckNotification({projectId: selectedRows[0]['projectId']}, (err, res)=>{
          if (err) {
            //case1.查询订单失败
 ...
 //----------------------------------end --------------------------------------------
  ...
  render() {
    ...
    const {selectedIds, selectedRows, spinning, tip,
      ...
      tableLoading, addPreApprovalT3Modal, viewPreApprovalT3Modal2,
      viewCountPreApproval,buttonloading1,buttonloading2, expandedRowKeys,activeKey, } = this.state;
      ...
      <div {...props} className="yzh-remained-submit-approval has-no-check-box-all">
        ...
        <Spin
          ...
          <Tabs  activeKey={activeKey}  onTabClick={this.onTabClick}>
              <Tabs.TabPane tab={<span><Icon type="tag" />导出excal</span>}  key="1">
              <GridTable
            ...
            onchange={this.handleTableChange}//排序
          />
              </Tabs.TabPane>
              <Tabs.TabPane tab={<span><Icon type="tag" />特殊物品</span>} key="2">
              <Table
...
CountApproval2.propTypes = {
  prefix: PropTypes.string,
  rowKey: PropTypes.string
};

CountApproval2.defaultProps = {
  prefix: 'yzh',
  rowKey: 'id'
};

CountApproval2 = Form.create()(CountApproval2);
...
export default connect(mapStateToProps)(CountApproval2);
