...
import {AddRoleModal,ViewPermissionsModal } from './modules/'
 ...
 * function  管理员之用户管理功能。
 **/
class RoleManaging extends Component{
...
//---------------------------------------------------- start 新增or修改 ------------------------------------------
  ...
  getCheckNotificationDetails = ({data}, callback)=>{ //根据id, 查询。
     fetch('ciq.role.selectbyid', {
       // method: 'post',
       // headers: {},
       data: {id: data.id},
       success: (res)=>{
  ...
  add = ({type,  data: selectedRows = []})=>{
    ...
    if(type === 'edit') {
      ...
      if (hasSelectedOnlyOneIdx) {
        ...
        this.getCheckNotificationDetails({data: selectedRows[0]}, (err, res={})=> { //根据id, 查询。
          if (err) {
            // case1. 请求通知明细失败
  ...
  stopOrderClick = ({data: selectedRows = []})=>{
    ...
    if (hasSelectedOnlyOneIdx) {
      ...
      if (isStatus0) {
        ...
        message.error('角色已被停用');
      }
    }
  };
  stopOrder = (selectedId = {}, callback = ()=>{})=>{ //1.根据id，停用用户。
    fetch('ciq.role.delete', {
      // method: 'post',
      // headers: {},
      data: selectedId,
      success: (res)=>{
...
//--------------------------------------------- start删除--------------------------------------
  ...
  resetOrderClick = ({data: selectedRows = []})=>{
    ...
    if(selectedRows.length === 0){
      // s1-case1. 没有选中任何行数据提示
      message.warn('你未选中任何记录');
      return;
    }else if(selectedRows.length > 1 ){
      // s1-case2. 选中超过2行数据提示
      message.warn('你只能选择一条记录')
      return;
    }
  ...
  submitOrder = (selectedId = {}, callback = ()=>{})=>{ //--------- 删除接口------------------------
    fetch('ciq.role.delete', {
      // method: 'post',
      // headers: {},
      data: selectedId,
...
//-----------------------------------------------start设置权限---------------------------------------------------
  ...
  viewOrder = (orderId = {}, callback = ()=>{})=>{  //根据id,查看用户权限。
    fetch('ciq.riskasesmtpermissions.pagelist', {
      // method: 'post',
      // headers: {},
      data: orderId,
      success: (res)=>{
  ...
  viewOrderClick =({dependentValues = {}})=> {
    // step1. 如果只选中一行数据，确认订单xhr
    this.showViewPermissionsModal({data: {}});
    this.viewOrder({id: dependentValues['id']}, (err, res)=> {
        if (err) {
          // s2-ss1-case1. 提交订单失败
          message.warn('你未选中任何记录');
          return;
        ...
        this.showViewPermissionsModal({data: responseObject});
    });

  }
  showViewPermissionsModal = ({data})=>{  //显示对话框
    this.setState(({viewPermissionsModal})=>{
      return {
  ...
  hiddenViewPermissionsModal = ()=>{ //隐藏对话框
    this.setState(({viewPermissionsModal})=>{
      return {
        viewPermissionsModal: {
          ...viewPermissionsModal,
  ...
  addViewOrderCallback = (info)=>{ //回调函数
    const {click, data, callback} = info;
    // 如果点击取消按钮，则隐藏对话框
    if(click === 'cancel'){
        this.hiddenViewPermissionsModal();
        callback && callback();
        return;
    }
    ...
    if(click === 'ok'){
        this.resetSelectedRowsNIds(); //清空选中行数据
        this.hiddenViewPermissionsModal();  //隐藏对话框
        callback && callback();
        const {listPage: {pageIndex, pageSize}} = this.state;  //重新请求列表数据。
        this.getCheckNotificationList({pageIndex, pageSize})
        return;
...
//---------------------------------------------------------------------------------------
  ...
  render() {
    ...
    const {selectedIds, selectedRows, spinning, tip,
      ...
      tableLoading, addRoleModal,viewPermissionsModal, } = this.state;
    ...
    const _columns = [
      ...
      },
      {
        ...
        formatter: ({dependentValues,rowIdx, value, column: {key}})=>{
          return (
            <ButtonContainer>
                  <Button type="primary" onClick={this.viewOrderClick.bind(this, {dependentValues})}>查看</Button>
            </ButtonContainer>
          )
        }
      },
    ...
    return (
      <div {...props} className="yzh-remained-submit-approval has-no-check-box-all">
        <div className="hidden">
          ...
          <ViewPermissionsModal
            {...viewPermissionsModal}
            callback={this.addViewOrderCallback}/>
        </div>
        ...
        <Spin
          ...
          tip={tip}>
          <ButtonContainer>
            <AddButton onClick={this.add.bind(this, {type: 'add', data:{}})}>新建角色</AddButton>
            <UploadButton onClick={this.add.bind(this, {type: 'edit', data: selectedRows})}>修改角色</UploadButton>
            <DelButton  onClick={this.resetOrderClick.bind(this, {data: selectedRows})}>删除角色</DelButton>
            {/*<EditButton onClick={this.resetOrderClick.bind(this, {data: selectedRows})}></EditButton>
...
RoleManaging.propTypes = {
  prefix: PropTypes.string,
  rowKey: PropTypes.string
};

RoleManaging.defaultProps = {
  prefix: 'yzh',
  rowKey: 'id'
};

RoleManaging = Form.create()(RoleManaging);
...
export default connect(mapStateToProps)(RoleManaging);
