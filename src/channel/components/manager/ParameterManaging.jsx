...
import {AddParameterModal, } from './modules/'
 ...
 * function  管理员之用户管理功能。
 **/
class ParameterManaging extends Component{
  ...
  getCheckNotificationList = (query)=>{
    ...
    //console.log('查询q:',_q);
    // step2. 请求列表数据
    fetch('ciq.riskasesmtparameter.pagelist',{ //
      // method: 'post',
      // headers: {},
      data: _q,
      success: (res)=>{
...
//---------------------------------------------------- start 新增or修改 ------------------------------------------
  ...
  getCheckNotificationDetails = ( selectedId ={}, callback)=>{ //根据id, 查询参数内容。
    fetch('ciq.riskasesmtparameter.selectbyid', {
      // method: 'post',
      // headers: {},
      data: selectedId,
      success: (res)=>{
  ...
  add = ({type,  data: selectedRows = []})=>{
    ...
    if(type === 'edit') {
      ...
      if (hasSelectedOnlyOneIdx) {
        ...
        this.getCheckNotificationDetails({id: selectedRows[0]['id']}, (err, res={})=> { //根据id, 查询。
          if (err) {
            // case1. 请求通知明细失败
...
//---------------------------------------------------end  查看申请单 ----------------------------------------
  ...
  dispatchPreApprovalCallback = (info)=>{ //查验查验通知明细信息的回调
...
//---------------------------------------------- end 重置密码  -------------------------------------
  ...
  render() {
    ...
    const {selectedIds, selectedRows, spinning, tip,
      ...
      tableLoading, addParameterModal, } = this.state;
    ...
    return (
      <div {...props} className="yzh-remained-submit-approval has-no-check-box-all">
        ...
        <Spin
          ...
          tip={tip}>
          <ButtonContainer>
            <EditButton onClick={this.add.bind(this, {type: 'edit', data: selectedRows})}>修改参数</EditButton>
...
ParameterManaging.propTypes = {
  prefix: PropTypes.string,
  rowKey: PropTypes.string
};

ParameterManaging.defaultProps = {
  prefix: 'yzh',
  rowKey: 'id'
};

ParameterManaging = Form.create()(ParameterManaging);
...
export default connect(mapStateToProps)(ParameterManaging);
